...
} from "@/app/actions/server/update-file-tags";
...
type EditFileTagsFieldsProps = {
  file: Pick<FileRecord, "id" | "name" | "tags">;
  onSuccess: () => void;
};

function EditFileTagsFields({ file, onSuccess }: EditFileTagsFieldsProps) {
  ...
  const [state, formAction, pending] = useActionState(
    async (_prev: UpdateFileTagsState, formData: FormData) => {
      const result = await updateFileTagsAction(initialState, formData);
      if (!result.error) {
        onSuccess();
      }
      return result;
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <label htmlFor={`add-single-tag-${file.id}`} className="text-sm font-medium">
          Add tag
        </label>
        <div className="flex gap-2">
          <input
            id={`add-single-tag-${file.id}`}
            type="text"
            autoComplete="off"
            value={newTag}
            disabled={pending}
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Toggle color tags, remove existing tags, and add one new tag at a time.
          </DialogDescription>
        </DialogHeader>
        <EditFileTagsFields key={instance} file={file} onSuccess={handleSuccess} />
      </DialogContent>
    </Dialog>
  );
}
